...
# select the clip to use. Set "None" for live recording
CLIP = f"{OUTPUT_DIR}/clip_04.mp4"
...
MIN_RADIUS  = 14
MAX_RADIUS  = 17
MIN_DIST_1  = MIN_RADIUS * 1.8 # for clahe detection
MIN_DIST_2  = MIN_RADIUS * 1.8 # for diff detection
MARGIN      = 42
ACQ_FRAMES  = 15
...
def draw_trajectory(frame_h, white_ball, cue_center, detected_circles):
    ...
    if hit_ball is not None and t_ball <= t_wall:
        ...
        # "Ghost" circle: shows where the white ball will hit the target
    ...
    # Case 2: wall hit before any ball
        ...
        cv2.line(frame_h, (wx1, wy1), (wx2, wy2), (0, 170, 200), 2)  # dimmer after bounce
...
writer = cv2.VideoWriter(f"{OUTPUT_DIR}/RESULTS/v13_rec_0{clip_count}.mp4", fourcc, delay, (OUTPUT_W, OUTPUT_H))
...
while True:
    ...
    if acquiring:
        ...
        diff_bgr = cv2.absdiff(frame_h, background_warped)
        ...
        if len(acq_buffer_gray) == ACQ_FRAMES:
            ...
            # Pass 2: background subtraction (colour-aware)
            # subtract the BGR background from the BGR frame,
            # then take the per-pixel maximum across the three channels.
    ...
    cv2.imshow("binary diff", binary_diff)
    ...
    if key == ord('q'):
        break
    if key == ord('s'):
        cv2.imwrite(f"{OUTPUT_DIR}/RESULTS/v13_frame_h_0{img_count}.png", frame_h)
        cv2.imwrite(f"{OUTPUT_DIR}/RESULTS/v13_binary_diff_0{img_count}.png", binary_diff)
        cv2.imwrite(f"{OUTPUT_DIR}/RESULTS/v13_display_clahe_0{img_count}.png", display_clahe)
        img_count += 1
        print(f"Saved images set {img_count}")
    if key == ord('r'):
        # save clip
        if not rec:
