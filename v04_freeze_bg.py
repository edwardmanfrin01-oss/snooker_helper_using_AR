...
PARAM2       = 14 # accumulator threshold
MIN_RADIUS   = 13
MIN_DIST     = MIN_RADIUS * 1.2
MAX_RADIUS   = 20
...
while True:
    ...
    if acquiring:
        ...
        acq_buffer.append(diff.astype(np.float32))
    ...
    if key == ord('s'):
        ...
        cv2.imwrite(f"{OUTPUT_DIR}/RESULTS/v04_binary_0{img_count}.png", binary)
        img_count+=1
        cv2.imwrite(f"{OUTPUT_DIR}/FREEZE_BG/v04_diff_0{img_count}.png", diff)
        img_count+=1
        cv2.imwrite(f"{OUTPUT_DIR}/FREEZE_BG/v04_output_0{img_count}.png", frame_homography)
        img_count+=1
